  ...
  private async sendRequest(method: string, params?: any): Promise<MCPResponse> {
    ...
    try {
      ...
      if (responseText.startsWith('event: message')) {
        ...
        // Extract session ID from the SSE event ID on initialize
        if (idLine && method === 'initialize' && !this.sessionId) {
          const eventId = idLine.substring(4); // Remove 'id: ' prefix
          
          // Try multiple session ID extraction strategies
          this.sessionId = eventId.split('_')[0]; // Use the UUID part before the first underscore
          console.log(`Extracted session ID: ${this.sessionId} from event ID: ${eventId}`);
        }
